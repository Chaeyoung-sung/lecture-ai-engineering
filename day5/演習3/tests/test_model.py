...
def train_model(sample_data, preprocessor):
    ...
    model_version_path = os.path.join(MODEL_HISTORY_DIR, f"titanic_model_{timestamp}.pkl")
...
def test_model_inference_time(train_model):
    ...
    # 推論時間が制限時間未満であることを確認
    assert avg_inference_time < MAX_INFERENCE_TIME, f"推論時間が長すぎます: {avg_inference_time:.6f}秒 > {MAX_INFERENCE_TIME}秒"
...
def test_compare_with_previous_model(train_model, sample_data):
    ...
    metrics_files.sort()
    previous_metrics_path = metrics_files[-2] if len(metrics_files) > 1 else metrics_files[0]
...
def test_model_feature_importance(train_model):
    ...
    top_two_importance_ratio = (importances_sorted[0] + importances_sorted[1]) / np.sum(importances)
...
def test_model_drift_detection(train_model, sample_data):
    ...
    # 両極端の値が存在するか確認
    assert max_proba - min_proba >= 0.5, f"予測確率の範囲が狭すぎます: {max_proba:.4f} - {min_proba:.4f} = {max_proba - min_proba:.4f}"
...
def test_model_serialization_format(train_model):
    ...
    try:
        ...
        # モデルが正しいクラスか確認
        assert isinstance(loaded_model, Pipeline), f"モデルが期待されるPipelineクラスではありません: {type(loaded_model)}"
    ...
    if len(metrics_files) > 1:
        ...
        try:
            ...
            for metric_name in metrics.keys():
                ...
                status = "\033[92m↑\033[0m" if change > 0 else "\033[91m↓\033[0m" if change < 0 else "-"
                print(f"  {metric_name.ljust(10)}: {current_val:.4f} ({status} {change_percent:+.1f}%)")
            
            # 主要指標が低下していないか確認
            for metric_name, threshold in [("accuracy", -0.05), ("f1", -0.05), ("roc_auc", -0.05)]:
                current_val = metrics[metric_name]
                previous_val = previous_metrics.get(metric_name, 0)
                change = current_val - previous_val
                ...
                # 10%以上の低下があれば失敗させる
                assert change >= -0.1, f"{metric_name}が大きく低下しています ({abs(change)*100:.1f}%)"
